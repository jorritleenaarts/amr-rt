...
namespace mem{
  namespace internal{
	...
	allocate(siz);
      ...
      M_INLINE explicit Storage(size_t const siz, T* idata): n_elements(siz), data(idata), allocated(false){};
      
      M_INLINE explicit Storage(Storage<T> const& istor):
	n_elements(istor.n_elements), data(istor.data), allocated(false){};
